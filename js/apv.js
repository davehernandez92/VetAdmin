...
class UI {
   ...
   imprimirCitas({citas}) { // Se puede aplicar destructuring desde la función...
        ...
        citas.forEach(cita => {
            ...
            sintomasParrafo.innerHTML = `<span class="font-weight-bolder">Sintoms: </span> ${sintomas}`;
   ...
   textoHeading(citas) {
        if(citas.length > 0 ) {
            heading.textContent = 'Admin your appointments '
        } else {
            heading.textContent = "There's no appointments yet..."
        }
    }
...
function nuevaCita(e) {
    ...
    if( mascota === '' || propietario === '' || telefono === '' || fecha === ''  || hora === '' || sintomas === '' ) {
        ui.imprimirAlerta('Please fill out all inputs', 'error')
    ...
    if(editando) {
        ...
        ui.imprimirAlerta('Saving...');

        formulario.querySelector('button[type="submit"]').textContent = 'Add an appointment';
        ...
        ui.imprimirAlerta('Added sucessfully')
    }
